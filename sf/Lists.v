...
Fixpoint nonzeros (l:natlist) : natlist :=
  match l with
  | nil => nil
  | h::t => if (eq_nat' h O) then nonzeros t
            else h::(nonzeros t)
  end.
...
Fixpoint count (v:nat) (s:bag) : nat := 
  match s with
  | nil => O
  | h::t => if (eq_nat' h v) then S (count v t)
            else (count v t)
  end.
...
Definition member (v:nat) (s:bag) : bool := 
  if (blt_nat O (count v s)) then true
  else false.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | h::t => if (eq_nat' h v) then (remove_all v t)
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  ...
  | h::t => match (remove_one h s1) with
            | nil => true
            | a => (subset a t)
            end
  end.
...
Lemma snoc_app_n : forall (l1 l2 : natlist) (n:nat),
...
Proof.
  intros. induction l1.
  - simpl. SearchPattern (_ ++ [] = _). rewrite app_nil_end. reflexivity.
  - simpl. rewrite IHl1. rewrite snoc_app_n. reflexivity.
Qed.
...
Proof.
  intros. induction l1.
  - simpl. reflexivity.
  - simpl. rewrite IHl1. (*You have to divide the 'if' here!!!*)destruct n.
    reflexivity.
    reflexivity.
Qed.
(** [] *)
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  ...
  | h::t => match l2 with
            | nil => false
            | p::q => if (eq_nat' h p) then (beq_natlist t q)
                      else false
            end
  end.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. destruct n.
    simpl. SearchAbout ble_nat. apply ble_n_Sn.
    simpl. apply IHs.
Qed.
(** [] *)
...
Fixpoint find (key : nat) (d : dictionary) : natoption := 
  match d with 
  | empty         => None
  | record k v d' => if (beq_nat key k)  then (Some v) 
                       else (find key d')
  end.
...
Proof.
 intros. induction d.
 - simpl. induction k. reflexivity. simpl. apply IHk.
 - simpl. induction k. reflexivity. simpl. SearchAbout beq_nat. rewrite <- beq_nat_refl. reflexivity.
Qed.
(** [] *)
